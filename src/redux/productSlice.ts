  ...
  reducers: {
    setFilter: (
      ...
      state.filteredProducts = state.allProducts.filter((p) => {
        ...
        if (state.filters.seller && p.sellerName !== state.filters.seller)
          return false;

        if (state.filters.location && p.location !== state.filters.location)
          return false;

        if (state.filters.category && p.itemName !== state.filters.category)
          return false;
